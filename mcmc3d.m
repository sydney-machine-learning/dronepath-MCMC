...
%     betanew = 10^betanew;
    ...
    ll(i+1) = loglikelihood(theta,sigsq, llf);
...
function Zs = loglikelihood(beta,sigsq, llf)
Zs = llf/2-(distance(beta)^2/(2*sigsq));
end
...
w = (subs(W,var,varval));
stepsize = min(1,12.5/delta)*min(1, delta/10)*0.05*min(1 , 0.02*exp(w/9)+exp(0.4*(w-9))); 
